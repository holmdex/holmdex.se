...
# Full region descriptions for display
region_descriptions <- c(
  "🇺🇸 US" = "United States (S&P 500, Nasdaq 100)",
  "🇪🇺 Europe" = "Europe (STOXX50E, FTSE, DAX, CAC40)",
  "🇳🇴 Nordics" = "Nordic Countries (Sweden, Denmark, Finland, Norway)",
  "🌏 Asia" = "Asia (Japan, Hong Kong, China, South Korea, India)",
  "🌍 Other" = "Other Markets (Brazil, Mexico, Canada, Saudi Arabia, South Africa)"
)
...
if (is.null(market_data)) {
  ...
  if(nrow(market_data) > 0) {
    market_data <- market_data %>%
      ...
      # Extract weekday as character to avoid factor level issues
      mutate(
...
# Define fixed weekday order to use throughout
weekday_order <- c("Mon", "Tue", "Wed", "Thu", "Fri")
...
# Calculate regional statistics to determine weights
regional_stats <- market_data %>%
...
# Calculate region weights based on data points
region_weights <- regional_stats %>%
  group_by(region) %>%
  summarize(
  ...
  mutate(
    weight_pct = total_points / sum(total_points) * 100
  )
...
# Create regional weight information for each weekday
# This is the critical fix - using character weekday to avoid factor level issues
regional_contributions <- regional_stats %>%
  left_join(region_weights, by = "region") %>%
  group_by(weekday) %>%
  summarize(
    region_weights = paste0(
      unique(region), ": ", 
      round(weight_pct, 1), "%", 
      collapse = ", "
    ),
...
# Join regional contributions to global stats using character weekday
global_weekday_stats <- global_weekday_stats %>%
  left_join(regional_contributions, by = "weekday") %>%
  # Now reorder weekdays properly after joining
...
# Create custom hover template with emojis, median and std dev
hover_template <- lapply(1:nrow(global_weekday_stats), function(i) {
  ...
  paste0(
    ...
    "Median Return: ", round(global_weekday_stats$median_return[i], 2), "%<br>",
    "Standard Deviation: ", round(global_weekday_stats$volatility[i], 2), "%<br>", 
    "Positive Days: ", round(global_weekday_stats$pct_positive[i], 1), "% of the time<br>",
    "<extra></extra>"
  )
})

# Format the regional weights with detailed descriptions
regional_weights_display <- paste("*Regional weights:", paste(
  sapply(names(region_weights$weight_pct), function(region) {
    paste0(region_descriptions[region], " - ", round(region_weights$weight_pct[region_weights$region == region], 1), "%")
  }),
  collapse = "; "
))

# Create enhanced global chart
global_chart <- plot_ly(
  ...
  layout(
    ...
    annotations = list(
      ...
      # Add regional weights as a subtitle annotation with more detailed descriptions
      list(
        xref = "paper",
        yref = "paper",
        x = 0.5,
        y = -0.15,
        text = regional_weights_display,
        showarrow = FALSE,
        font = list(size = 12, color = "gray", italic = TRUE)
      )
    ),
...
Different markets can show different patterns. Use the dropdown menu to select a market index:
...
index_weekday_stats <- market_data_filtered %>%
  ...
  # Use the same weekday ordering as the global chart
  mutate(weekday = factor(weekday, levels = weekday_order))

# Group indices by region for better organization
region_indices <- market_data_filtered %>%
  select(region, index_name) %>%
  distinct() %>%
  arrange(region, index_name)

# Create button labels that include region headers
button_labels <- c()
for (region in unique(region_indices$region)) {
  # Add a region separator
  button_labels <- c(button_labels, paste0("📍 ", region, " 📍"))
  
  # Add all indices from this region
  region_label_indices <- region_indices %>%
    filter(region == !!region) %>%
    pull(index_name)
  
  button_labels <- c(button_labels, region_label_indices)
}

# Get actual indices (without separators)
available_indices <- region_indices$index_name

# Create buttons for the dropdown
index_buttons <- lapply(seq_along(button_labels), function(i) {
  label <- button_labels[i]
  
  # Check if this is a region separator
  if (startsWith(label, "📍")) {
    # Return a non-functional button for the region header
    return(list(
      method = "skip",
      label = label,
      args = list(list(), list())
  ...
  # This is an actual index - create a functional button
  idx <- label
  ...
  # Create button with full index name as the label
  list(
    method = "update",
    args = list(
      # Trace update
...
# Create plot with dropdown menu
index_chart <- plot_ly(
  data = first_idx_data,
  x = ~weekday,
  y = ~mean_return,
  ...
  hovertemplate = paste(
    ...
    "Standard Deviation: %{customdata[2]:.2f}%<br>",
  ...
  customdata = cbind(
    ...
    first_idx_data$median_return,
    first_idx_data$volatility
  )
) %>%
  layout(
    title = list(
