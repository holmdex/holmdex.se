...
get_market_data <- function(symbol, start_date, end_date, retries = 5, timeout = 60) {
  ...
  for(retry in 1:retries) {
    ...
    # Method 9: Try with an even shorter time period (3 year)
...
```
```{r load-market-data}
# Define constants for data processing if not already defined
if (!exists("MAX_RETURN_VALUE")) MAX_RETURN_VALUE <- 20  # Cap at ±20% for daily returns
if (!exists("MAX_REGIONAL_RETURN")) MAX_REGIONAL_RETURN <- 5  # Cap at ±5% for regional averages
if (!exists("WINSORIZE_PERCENTILE")) WINSORIZE_PERCENTILE <- 0.005  # Winsorize at 0.5% 

# Skip data fetching if we already loaded from cache
if(is.null(market_data)) {
  ...
  if(is.null(market_data)) {
    ...
    tryCatch({
      ...
      # Also save to dated cache files
      if (!dir.exists("data")) {
        dir.create("data", showWarnings = FALSE)
      }
      
      # Save to weekly and daily cache files for GitHub Actions
      saveRDS(market_data, file.path("data", paste0("global_market_data_cache_5yr_", format(Sys.Date(), "%Y_%W"), ".rds")))
      saveRDS(market_data, file.path("data", paste0("global_market_data_cache_5yr_", format(Sys.Date(), "%Y_%m_%d"), ".rds")))
...
# Define a robust weekday calculation function that works across different locales
get_standardized_weekday <- function(date) {
  # First try the standard lubridate/base R approach
  weekday_num <- as.integer(format(date, "%u"))  # 1=Monday, 7=Sunday
  
  # Handle potential errors with locale or NA values
  if (is.na(weekday_num) || weekday_num < 1 || weekday_num > 7) {
    # Fallback to manual calculation using the day of the week
    weekday_num <- lubridate::wday(date, week_start = 1)  # Ensure Monday = 1
  }
  
  # Map to standard 3-letter abbreviations
  weekday_abbr <- c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")[weekday_num]
  
  # For trading data, we only want weekdays 1-5 (Mon-Fri)
  if (weekday_num <= 5) {
    return(weekday_abbr)
  } else {
    return(NA)  # Weekend days
  }
}
...
# Add a fallback mechanism when all data fetching methods fail
if (nrow(market_data) == 0 || !exists("market_data") || is.null(market_data)) {
  warning("Unable to fetch market data through normal methods. Checking for fallback data...")
  
  # Look for any cache files in the data directory
  cache_files <- list.files("data", pattern = "global_market_data_cache", full.names = TRUE)
  
  for (file in cache_files) {
    tryCatch({
      message(paste("Trying to load fallback data from", file))
      fallback_data <- readRDS(file)
      
      if (!is.null(fallback_data) && nrow(fallback_data) > 0 && 
          all(c("date", "symbol", "close") %in% colnames(fallback_data))) {
        message(paste("Successfully loaded fallback data with", nrow(fallback_data), "rows from", file))
        market_data <- fallback_data
        break
      }
    }, error = function(e) {
      message(paste("Error loading fallback data from", file, ":", e$message))
    })
  }
  
  # If we still don't have data, show a clear error message
  if (nrow(market_data) == 0 || !exists("market_data") || is.null(market_data)) {
    stop("
    CRITICAL ERROR: Unable to fetch or load any market data.
    Please check the following:
    1. Network connectivity and access to financial APIs
    2. Existence of cache files in the data directory
    3. System resources (memory, CPU)
    4. Permissions for reading/writing to the data directory
    
    Try running the report locally first to establish a cache file that can be committed to the repository.
    ")
  }
}
...
# Validate the market data structure before analysis
validate_market_data <- function(data) {
  # Check for required columns
  required_cols <- c("date", "symbol", "close", "region", "index_name", "weekday", "winsorized_return")
  missing_cols <- setdiff(required_cols, colnames(data))
  
  if (length(missing_cols) > 0) {
    stop(paste("Market data is missing required columns:", paste(missing_cols, collapse=", ")))
  }
  
  # Check for sufficient data volume
  if (nrow(data) < 1000) {
    warning(paste("Market data has only", nrow(data), "rows, which may be insufficient for analysis"))
  }
  ...
  if (date_span < 365) {
    warning(paste("Market data spans only", date_span, "days, which may be too short for reliable analysis"))
  }
  
  # Check for weekday distribution
  weekday_counts <- table(data$weekday)
  min_weekday_count <- min(weekday_counts)
  
  if (min_weekday_count < 50) {
    warning(paste("Some weekdays have very few observations. Minimum count:", min_weekday_count))
  }
  
  # Validate numeric columns
  numeric_cols <- c("close", "winsorized_return")
  for (col in numeric_cols) {
    if (sum(is.na(data[[col]])) > nrow(data) * 0.1) {
      warning(paste("Column", col, "has", sum(is.na(data[[col]])), "NA values (", 
                   round(sum(is.na(data[[col]])) / nrow(data) * 100, 1), "%)"))
    }
  }
  
  # Validate factor columns
  if (!"weekday" %in% names(data) || !is.factor(data$weekday)) {
    warning("Column 'weekday' should be a factor but isn't")
    # Try to fix it
    data$weekday <- factor(data$weekday, levels = c("Mon", "Tue", "Wed", "Thu", "Fri"))
  }
  
  # Check weekday levels
  if (is.factor(data$weekday)) {
    expected_levels <- c("Mon", "Tue", "Wed", "Thu", "Fri")
    missing_levels <- setdiff(expected_levels, levels(data$weekday))
    
    if (length(missing_levels) > 0) {
      warning(paste("Weekday factor is missing levels:", paste(missing_levels, collapse=", ")))
      # Try to fix it
      data$weekday <- factor(data$weekday, levels = expected_levels)
    }
  }
...
# Run validation on the market data
market_data <- validate_market_data(market_data)

# Print validation success message
message("Market data successfully validated with ", nrow(market_data), " rows spanning ", 
       length(unique(market_data$date)), " trading days")
message("Date range: ", paste(range(market_data$date), collapse=" to "))
message("Regions: ", paste(unique(market_data$region), collapse=", "))
message("Indices: ", paste(unique(market_data$index_name), collapse=", "))
```
